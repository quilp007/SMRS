...
class qt(QMainWindow, form_class):
    def __init__(self):
        ...
        btb heat on connect func_btn_hea_on(btb heat on
        btb heat on and stop  connect func_btn_hea_on(btb heat on and stop


        btb pre heat on connect func_btn_pre_hea_on(btb pre heat on
        btb pre heat on and stop connect func_btn_pre_hea_on(btb pre heat on and stop

 ddddaaddfddadfadfaadfadfad
aaaaaaaaa ddd ddd ddd ddd dd d ddsssaaaaaaaaaaasas``
    ...
    def check_temp(self):
        ...
        if self.config_dict['air_temp'] <= self.config_dict['set_air_temp']:
            ...
            self.func.btn.pre.heat.on(self.btn.pre.heat.on)
    ...
    def heat.tiimeout.func(self, inLabel)
        if inLael == self.btn.heat.stop
            self.heat.auto.mode = false
            self.btn.heat.auto.mode.setstylesheet(gray)

        self.pre.heat.timer.stop
        self.heat.timer.stop

        self.pre.heat.status =  false
        self.heat.status = false
        self.flag.em.heat.on  = false
        self.flag.heat.heat.on  =  false

        self.tn.pre.heat.on.set(gray)
        self.btn.pre.heat.on.adn.stop.set(gray)
        self.btn..heat.on.set(gray)
        self.btn.heat.on.and.stop.set(gray)

        self.label_pre.heat.on.set(gray)
        self.label_heat.on.set(gray)
        self.label_.heat.on.set(gray)
        ...
        self.label_warning.timerr..stop
        self.label.warig.setvisible(false)
    ...
    def func.heat.auto.mode
        self.heat.auto.mode = true
        self.btn.heat.auto.mode.set(blue)

    def func.btn.pre.heat.on:
        if self.flag.heat.on:
            return

        self.label.emc.heat.on.set(ggray)
        self.label.pre.heat.on.set(yellow)

        if button == self.btn.pre.heat.on:
            self.heat.auto.mode = True
            self.btn.heat.auto.mode.set(blue)
            self.btn.pre.heat.on.set(yellow)
        elif button == self.btn.pre.heat.on.and.stop:
            self.heat.auto.mode = False
            self.btn.heat.auto.mode.set(gray)
            self.btn.pre.heat.on.and.atop.set(yello)

        self.pre.heat.timerr.start(pre.heatingtime)

        self.flag.heat.on = True
        self.pre.heat.status = True
        self.heat_status = False
    ...
    def fucn.btn.heat.on(self, button):
        if self.flag.heat.on:
            return
        ...
        if button == self.btn.heat.on:
            self.heat.auto.mode = True
            self.btn.heat.auto.mode.set(blue)
            self.btn.heat.on.set(pink)
        elif button == self.btn.heat.on.and.stop:
            self.heat.auto.mode = False
            self.btn.heat.auto.mode.set(gray)
            self.btn.heat.on.and.atop.set(pink)

        self.heat.timerr.start(heatingtime)
