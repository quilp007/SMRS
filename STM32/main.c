...
{
	HAL_UART_Transmit(&huart1, p, len, 500);
...
cJSON *cJSON_CreateNumber_test(double num) {cJSON *item=cJSON_New_Item();if(item){item->type=cJSON_Number;item->valuedouble=num;}return item;}
void create_objects(void)
{
	...
	cJSON_AddItemToObject(root,"test",cJSON_CreateString("test for code"));
	cJSON_AddItemToObject( root, "y", cJSON_CreateNumber_test(2) );
	cJSON_AddItemToObject( root, "x", cJSON_CreateNumber_test(3) );
